...
<p>Записей в&nbsp;логе: <b class="col"><?=$entries_num?></b>.
<? if ($entries_num > 0) { ?>
...
Из них показано: <b class="col"><span id="shown_num"><?=$entries_shown?></span></b>.&nbsp;&nbsp;<br>
...
<td width="10">&nbsp;</td>
<td>
<b>IP:</b> <span id="ip[<?=$key?>]"><?=$sniffed['ip']?></span> <small>(<a href="http://nic.ru/whois/?ip=<?=$sniffed['ip']?>" target="_blank">whois</a>)</small><? if (!empty($sniffed['real_ip'])) { ?> &nbsp;&nbsp;<b>Real IP<a href="javascript:realIPInfo();">*</a>:</b> <?=$sniffed['real_ip']?> <small>(<a href="http://nic.ru/whois/?ip=<?=$sniffed['real_ip']?>" target="_blank">whois</a>)</small><? } ?><br>
<b>QUERY:</b> <span id="query[<?=$key?>]"><?=$sniffed['query']?></span><br>
<b>REFERER:</b> <? if ($sniffed['referer']) { ?><a href="<?=$sniffed['referer']?>" target="_blank"><?=$sniffed['referer']?></a><? } ?><br>
<b>AGENT:</b> <span id="agent[<?=$key?>]"><?=$sniffed['agent']?></span><br>
</td>
</tr>
</table>
<br>
...
<? } ?>
<p id="copyright">
php Based Sniffer 4.1 Public<br>
